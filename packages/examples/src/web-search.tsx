...
function TechnicalWebSearch({ maxSearches = 3 }: { maxSearches?: number }) {
  useEffect(() => {
    console.log(
      `🔧 [TechnicalWebSearch] Mounted (max ${maxSearches} searches)`
    );
...
function SearchActivityTracker() {
  ...
  useEffect(() => {
    ...
    if (searchCalls.length > 0) {
      console.log(
        `🔎 [Activity] Web searches performed: ${searchCalls.length}`
      );
    }
  }, [messages.length]);
...
function TechnicalResearcher() {
  ...
  return (
    <Agent model={MODEL} maxTokens={4096}>
      <System>
        You are a technical documentation researcher specializing in JavaScript/TypeScript ecosystems.

        AVAILABLE SOURCES (domain-restricted):
        - github.com, npmjs.com, typescriptlang.org, react.dev, nodejs.org, stackoverflow.com

        WORKFLOW:
        1. Use web_search to find information from technical documentation
        2. Extract specific facts and code examples
        3. Use summarize_findings to create a structured summary
        4. Provide links to the most relevant sources

        Focus on official documentation and well-established community resources.
      </System>
      ...
      <Message role="user">
        Research how to use React Server Components with TypeScript.

        Please:
        1. Search for official documentation and examples
        2. Find key implementation details
        3. Summarize your findings with confidence level
