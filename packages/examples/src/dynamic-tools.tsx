...
function MathAgent() {
  ...
  return (
    <Agent model="claude-haiku-4-5" maxTokens={2048}>
        ...
        {!isUnlocked && (
          <Tool
            {...defineTool({
              ...
              description: 'unlock advanced math operations (multiply, divide, power)',
              parameters: z.object({
        ...
        {isUnlocked && (
          <Tool
            {...defineTool({
              ...
              parameters: z.object({
                operation: z.enum(['multiply', 'divide', 'power']).describe('the operation to perform'),
                a: z.number().describe('first number'),
                b: z.number().describe('second number'),
              }),
              handler: async ({ operation, a, b }) => {
                switch (operation) {
                  case 'multiply':
                    return `${a} × ${b} = ${a * b}`;
                  case 'divide':
                    return b === 0 ? 'Error: division by zero' : `${a} ÷ ${b} = ${a / b}`;
                  case 'power':
      ...
      <Message role="user">
        Add 15 + 27, then unlock advanced features and calculate 6 to the power of 3.
      </Message>
    </Agent>
  );
}
