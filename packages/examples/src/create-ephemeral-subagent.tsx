  ...
  return (
    <Agent
      ...
      onComplete={() => {
        ...
        // Remove from state - reconciler will automatically handle tool removal
        onRemove(); // Subagent kills itself
      }}
    >
  ...
  return (
    <Tool
      ...
      inputSchema={z.object({
        name: z.string().describe('Subagent name in lowercase with underscores, e.g. "calculator" (required)'),
        description: z.string().describe('Brief description of what the subagent does (required)'),
        systemPrompt: z.string().describe('Complete system prompt/instructions that define the subagent\'s behavior (required)'),
...
function MainAgent() {
  ...
  return (
    ...
    >
      <System>
        You can spawn temporary subagents that will automatically remove themselves when they complete their task.

        IMPORTANT: Once a subagent completes its task, it automatically removes itself and is NO LONGER AVAILABLE. 
        If you try to call it again, the tool will not exist.

        WORKFLOW:
        1. Use create_subagent to create a subagent with a name, description, and system prompt
        2. The subagent will appear as a tool you can call
        3. Call the subagent tool with a task
        4. The subagent will automatically remove itself after completing its task
        5. After removal, the tool will no longer be available
      ...
      <Message role="user">
        Please do the following to demonstrate ephemeral subagents:
        
        1. Create a calculator subagent using create_subagent
        2. Use the calculator tool to calculate: (15 * 23) + (42 / 7) - 8
        3. After the calculator completes, try to call the calculator tool AGAIN with a different calculation: 100 / 4
        
        The second call should fail because the calculator subagent will have automatically removed itself after the first task. This proves that once a subagent unmounts, you no longer have access to it.
      </Message>
    </Agent>
  );
}
